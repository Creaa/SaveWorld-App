      ...
      >
        {this.props.citiesList.map((el, k) => {
          return (
            <li
              city={el.city}
              onClick={this.elementClickHander}
              className="ranking-element"
              style={{
                backgroundColor: this.setAirCondition(
                  parseFloat(el.value).toFixed(2)
                )
              }}
              key={k}
            >
              <p city={el.city} className="element-city">
                {el.city}
              </p>
              <span city={el.city} className="element-parameters">
                {parseFloat(el.value).toFixed(2)} {el.unit}
              </span>
              <div className="description-bubble">
                <img
                  onMouseOver={this.showItem}
                ...
                <div
                  style={{
                    visibility:
                      this.state.cityDisplayed === el.city
                        ? "visible"
                        : "hidden"
                  }}
                  className="bubble"
              ...
              <p className="city-description">{el.description}</p>
            </li>
          );
        })}
      </ul>
